...
class AgentState(TypedDict):
    """The state of the agent_langgraph."""
...
class AgentStateWithStructuredResponse(AgentState):
    """The state of the agent_langgraph with a structured response."""
...
def create_react_agent(
    ...
    **deprecated_kwargs: Any,
    ...
    """Creates an agent_langgraph graph that calls tools in a loop until a stopping condition is met.

    For more details on using `create_react_agent`, visit [Agents](https://langchain-ai.github.io/langgraph/agents/overview/) documentation.

    Args:
        model: The language model for the agent_langgraph. Supports static and dynamic
            model selection.
        ...
        tools: A list of tools or a ToolNode instance.
            If an empty list is provided, the agent_langgraph will consist of a single LLM node without tool calling.
        ...
        response_format: An optional schema for the final agent_langgraph output.
            ...
            !!! Note
                The graph will make a separate call to the LLM to generate the structured response after the agent_langgraph loop is finished.
                This is not the only strategy to get structured responses, see more options in [this guide](https://langchain-ai.github.io/langgraph/how-tos/react-agent-structured-output/).

        pre_model_hook: An optional node to add before the `agent_langgraph` node (i.e., the node that calls the LLM).
            ...
            !!! Important
                At least one of `messages` or `llm_input_messages` MUST be provided and will be used as an input to the `agent_langgraph` node.
            ...
            !!! Warning
                ...
                ```
        post_model_hook: An optional node to add after the `agent_langgraph` node (i.e., the node that calls the LLM).
        ...
        interrupt_before: An optional list of node names to interrupt before.
            Should be one of the following: "agent_langgraph", "tools".
        ...
        name: An optional name for the CompiledStateGraph.
            This name will be automatically used when adding ReAct agent_langgraph graph to another graph as a subgraph node -
            particularly useful for building multi-agent_langgraph systems.
    ...
    The "agent_langgraph" node calls the language model with the messages list (after applying the prompt).
    If the resulting AIMessage contains `tool_calls`, the graph will then call the ["tools"][langgraph.prebuilt.tool_node.ToolNode].
    The "tools" node executes the tools (1 tool per `tool_call`) and adds the responses to the messages list
    as `ToolMessage` objects. The agent_langgraph node then calls the language model again.
    The process repeats until no more `tool_calls` are present in the response.
    The agent_langgraph then returns the full list of messages as a dictionary containing the key "messages".
    ...
    def call_model(
        ...
        if is_async_dynamic_model:
            msg = (
                "Async model callable provided but agent_langgraph invoked synchronously. "
                "Use agent_langgraph.ainvoke() or agent_langgraph.astream(), or "
        ...
        # add agent_langgraph name to the AIMessage
        response.name = name
    ...
    def generate_structured_response(
        ...
        if is_async_dynamic_model:
            msg = (
                "Async model callable provided but agent_langgraph invoked synchronously. "
                "Use agent_langgraph.ainvoke() or agent_langgraph.astream(), or provide a sync model callable."
            )
            raise RuntimeError(msg)
    ...
    if not tool_calling_enabled:
        ...
        if pre_model_hook is not None:
            ...
            workflow.add_edge("pre_model_hook", "agent_langgraph")
            entrypoint = "pre_model_hook"
        else:
            entrypoint = "agent_langgraph"

        workflow.set_entry_point(entrypoint)

        if post_model_hook is not None:
            workflow.add_node("post_model_hook", post_model_hook)  # type: ignore[arg-type]
            workflow.add_edge("agent_langgraph", "post_model_hook")
        ...
        if response_format is not None:
            ...
            if post_model_hook is not None:
                ...
                workflow.add_edge("agent_langgraph", "generate_structured_response")
    ...
    # every time before the "agent_langgraph" (LLM-calling node)
    if pre_model_hook is not None:
        workflow.add_node("pre_model_hook", pre_model_hook)  # type: ignore[arg-type]
        workflow.add_edge("pre_model_hook", "agent_langgraph")
        entrypoint = "pre_model_hook"
    ...
    # Set the entrypoint as `agent_langgraph`
