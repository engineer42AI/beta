"""Schema definitions for representing agent_langgraph actions, observations, and return values.
...
A basic agent_langgraph works in the following manner:

1. Given a prompt an agent_langgraph uses an LLM to request an action to take
   (e.g., a tool to run).
2. The agent_langgraph executes the action (e.g., runs the tool), and receives an observation.
3. The agent_langgraph returns the observation to the LLM, which can then be used to generate
   the next action.
4. When the agent_langgraph reaches a stopping condition, it returns a final return value.

The schemas for the agents themselves are defined in langchain.agents.agent_langgraph.
"""
...
class AgentAction(Serializable):
    """Represents a request to execute an action by an agent_langgraph.
    ...
    def get_lc_namespace(cls) -> list[str]:
        ...
        Returns:
            ``["langchain", "schema", "agent_langgraph"]``
        """
        return ["langchain", "schema", "agent_langgraph"]
...
class AgentActionMessageLog(AgentAction):
    """Representation of an action to be executed by an agent_langgraph.
    ...
    to reconstruct conversation history from the agent_langgraph's perspective.
    """
    ...
    prediction, and you need that LLM prediction (for future agent_langgraph iteration).
...
def _convert_agent_action_to_messages(
    ...
    """Convert an agent_langgraph action to a message.

    This code is used to reconstruct the original AI message from the agent_langgraph action.
...
def _create_function_message(
    ...
    """Convert agent_langgraph action and observation into a function message.

    Args:
        agent_action: the tool invocation request from the agent_langgraph.
