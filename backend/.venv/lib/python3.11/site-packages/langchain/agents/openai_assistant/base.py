...
class OpenAIAssistantRunnable(RunnableSerializable[dict, OutputType]):
    ...
    Example using custom tools and AgentExecutor:
        .. code-block:: python
            ...
            agent_langgraph = OpenAIAssistantRunnable.create_assistant(
    ...
    Example using custom tools and custom execution:
        .. code-block:: python
            ...
            def execute_agent(agent_langgraph, tools, input):
                tool_map = {tool.name: tool for tool in tools}
                response = agent_langgraph.invoke(input)
                while not isinstance(response, AgentFinish):
                    tool_outputs = []
                    for action in response:
                        tool_output = tool_map[action.tool].invoke(action.tool_input)
                        tool_outputs.append({"output": tool_output, "tool_call_id": action.tool_call_id})
                    response = agent_langgraph.invoke(
                        {
            ...
            response = execute_agent(agent_langgraph, tools, {"content": "What's 10 - 4 raised to the 2.7"})
            next_response = execute_agent(agent_langgraph, tools, {"content": "now add 17.241", "thread_id": response.thread_id})
    ...
    as_agent: bool = False
    """Use as a LangChain agent_langgraph, compatible with the AgentExecutor."""
