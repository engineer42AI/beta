...
def create_vectorstore_agent(
    ...
    **kwargs: Any,
    ...
    """Construct a VectorStore agent_langgraph from an LLM and tools.
    ...
    calling methods and LangGraph. Install LangGraph with:
        ...
        .. code-block:: python
            ...
            agent_langgraph = create_react_agent(llm, [tool])

            for step in agent_langgraph.stream(
    ...
    Args:
        llm (BaseLanguageModel): LLM that will be used by the agent_langgraph
        toolkit (VectorStoreToolkit): Set of tools for the agent_langgraph
        callback_manager (Optional[BaseCallbackManager], optional): Object to handle the callback [ Defaults to None. ]
        prefix (str, optional): The prefix prompt for the agent_langgraph. If not provided uses default PREFIX.
        verbose (bool, optional): If you want to see the content of the scratchpad. [ Defaults to False ]
        agent_executor_kwargs (Optional[Dict[str, Any]], optional): If there is any other parameter you want to send to the agent_langgraph. [ Defaults to None ]
...
def create_vectorstore_router_agent(
    ...
    **kwargs: Any,
    ...
    """Construct a VectorStore router agent_langgraph from an LLM and tools.
    ...
    calling methods and LangGraph. Install LangGraph with:
        ...
        .. code-block:: python
            ...
            agent_langgraph = create_react_agent(llm, tools)
    ...
    Args:
        llm (BaseLanguageModel): LLM that will be used by the agent_langgraph
        toolkit (VectorStoreRouterToolkit): Set of tools for the agent_langgraph which have routing capability with multiple vector stores
        callback_manager (Optional[BaseCallbackManager], optional): Object to handle the callback [ Defaults to None. ]
        prefix (str, optional): The prefix prompt for the router agent_langgraph. If not provided uses default ROUTER_PREFIX.
        verbose (bool, optional): If you want to see the content of the scratchpad. [ Defaults to False ]
        agent_executor_kwargs (Optional[Dict[str, Any]], optional): If there is any other parameter you want to send to the agent_langgraph. [ Defaults to None ]
        kwargs: Additional named parameters to pass to the ZeroShotAgent.
