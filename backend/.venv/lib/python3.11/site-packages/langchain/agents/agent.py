...
class BaseSingleActionAgent(BaseModel):
    ...
    def return_values(self) -> list[str]:
        """Return values of the agent_langgraph."""
        return ["output"]
    ...
    def return_stopped_response(
        ...
        **kwargs: Any,
        ...
        """Return response when agent_langgraph has been stopped due to max iterations.
    ...
    def from_llm_and_tools(
        ...
        **kwargs: Any,
        ...
        """Construct an agent_langgraph from an LLM and tools.
    ...
    def _agent_type(self) -> str:
        """Return Identifier of an agent_langgraph type."""
        raise NotImplementedError

    def dict(self, **kwargs: Any) -> builtins.dict:
        """Return dictionary representation of agent_langgraph.

        Returns:
            Dict: Dictionary representation of agent_langgraph.
        """
    ...
    def save(self, file_path: Union[Path, str]) -> None:
        """Save the agent_langgraph.

        Args:
            file_path: Path to file to save the agent_langgraph to.

        Example:
        .. code-block:: python

            # If working with agent_langgraph executor
            agent_langgraph.agent_langgraph.save(file_path="path/agent_langgraph.yaml")
        """
...
class BaseMultiActionAgent(BaseModel):
    ...
    def save(self, file_path: Union[Path, str]) -> None:
        ...
        Raises:
            NotImplementedError: If agent_langgraph does not support saving.
...
class AgentOutputParser(BaseOutputParser[Union[AgentAction, AgentFinish]]):
    """Base class for parsing agent_langgraph output into agent_langgraph action/finish."""

    @abstractmethod
    def parse(self, text: str) -> Union[AgentAction, AgentFinish]:
        """Parse text into agent_langgraph action/finish."""


class MultiActionAgentOutputParser(
    BaseOutputParser[Union[list[AgentAction], AgentFinish]],
):
    """Base class for parsing agent_langgraph output into agent_langgraph actions/finish.
    ...
    def parse(self, text: str) -> Union[list[AgentAction], AgentFinish]:
        """Parse text into agent_langgraph actions/finish.
        ...
        Returns:
            Union[List[AgentAction], AgentFinish]:
                List of agent_langgraph actions or agent_langgraph finish.
        """
...
class RunnableAgent(BaseSingleActionAgent):
    ...
    """Runnable to call to get agent_langgraph action."""
...
class RunnableMultiActionAgent(BaseMultiActionAgent):
    ...
    """Runnable to call to get agent_langgraph actions."""
    input_keys_arg: list[str] = []
    return_keys_arg: list[str] = []
    stream_runnable: bool = True
    """Whether to stream from the runnable or not.
...
class LLMSingleActionAgent(BaseSingleActionAgent):
    ...
    llm_chain: LLMChain
    """LLMChain to use for agent_langgraph."""
    output_parser: AgentOutputParser
    """Output parser to use for agent_langgraph."""
    stop: list[str]
...
class Agent(BaseSingleActionAgent):
    ...
    a variable called "agent_scratchpad" where the agent_langgraph can put its
    intermediary work.
    """
    ...
    """Allowed tools for the agent_langgraph. If None, all tools are allowed."""
    ...
    def _fix_text(self, text: str) -> str:
        ...
        Returns:
            str: Fixed text.
        """
        msg = "fix_text not implemented for this agent_langgraph."
        raise ValueError(msg)
    ...
    def _construct_scratchpad(
        ...
        """Construct the scratchpad that lets the agent_langgraph continue its thought process."""
        thoughts = ""
...
class AgentExecutor(Chain):
    ...
    """The agent_langgraph to run for creating a plan and determining actions
    to take at each step of the execution loop."""
    tools: Sequence[BaseTool]
    """The valid tools the agent_langgraph can call."""
    return_intermediate_steps: bool = False
    """Whether to return the agent_langgraph's trajectory of intermediate steps
    ...
    """The method to use for early stopping if the agent_langgraph never
    returns `AgentFinish`. Either 'force' or 'generate'.

    `"force"` returns a string saying that it stopped because it met a
        time or iteration limit.

    `"generate"` calls the agent_langgraph's LLM Chain one final time to generate
        a final answer based on the previous steps.
    """
    handle_parsing_errors: Union[bool, str, Callable[[OutputParserException], str]] = (
        False
    )
    """How to handle errors raised by the agent_langgraph's output parser.
    Defaults to `False`, which raises the error.
    If `true`, the error will be sent back to the LLM as an observation.
    If a string, the string itself will be sent to the LLM as an observation.
    If a callable function, the function will be called with the exception
     as an argument, and the result of that function will be passed to the agent_langgraph
      as an observation.
    """
    ...
    def from_agent_and_tools(
        ...
        **kwargs: Any,
        ...
        """Create from agent_langgraph and tools.
    ...
    def validate_tools(self) -> Self:
        """Validate that tools are compatible with agent_langgraph.
    ...
    def validate_runnable_agent(cls, values: dict) -> Any:
        """Convert runnable to agent_langgraph if passed in.
        ...
        Returns:
            Dict: Validated values.
        """
        agent = values.get("agent_langgraph")
        if agent and isinstance(agent, Runnable):
            ...
            if multi_action:
                values["agent_langgraph"] = RunnableMultiActionAgent(
                    runnable=agent,
                    stream_runnable=stream_runnable,
                )
            else:
                values["agent_langgraph"] = RunnableAgent(
                    runnable=agent,
                    stream_runnable=stream_runnable,
                )
        return values
    ...
    def _action_agent(self) -> Union[BaseSingleActionAgent, BaseMultiActionAgent]:
        """Type cast self.agent_langgraph.

        If the `agent_langgraph` attribute is a Runnable, it will be converted one of
    ...
    def save(self, file_path: Union[Path, str]) -> None:
        ...
        Raises:
            ValueError: Saving not supported for agent_langgraph executors.
        """
        msg = (
            "Saving not supported for agent_langgraph executors. "
            "If you are trying to save the agent_langgraph, please use the "
            "`.save_agent(...)`"
        )
        raise ValueError(msg)

    def save_agent(self, file_path: Union[Path, str]) -> None:
        """Save the underlying agent_langgraph.
    ...
    def iter(
        ...
        *,
        ...
        Args:
            inputs: Inputs to the agent_langgraph.
            callbacks: Callbacks to run.
    ...
    def _iter_next_step(
        ...
        Override this to take control of how the agent_langgraph makes and acts on choices.
        """
        try:
            ...
            if raise_error:
                msg = (
                    ...
                    "In order to pass this error back to the agent_langgraph and have it try "
    ...
    def _call(
        ...
        """Run text through and get agent_langgraph response."""
        ...
        # We now enter the agent_langgraph loop (until it returns something).
    ...
    async def _acall(
        ...
        """Async run text through and get agent_langgraph response."""
        # Construct a mapping of tool name to tool for easy lookup
        name_to_tool_map = {tool.name: tool for tool in self.tools}
        # We construct a mapping from each tool to a color, used for logging.
        color_mapping = get_color_mapping(
    ...
    def stream(
        ...
        **kwargs: Any,
        ...
        Args:
            input: Input to the agent_langgraph.
